...
class TestParameterSetUnpack:
    """Test ParameterSet.unpack() method for multi-value extraction.

    This test class verifies the unpack() helper that extracts multiple
    parameter values in a single call, reducing boilerplate in model
    implementations.
    """

    @pytest.fixture
    def params(self):
        """Create a ParameterSet with test parameters."""
        ps = ParameterSet()
        ps.add("x", value=1.5)
        ps.add("G0", value=100.0)
        ps.add("tau0", value=0.01)
        ps.add("alpha", value=0.5)
        return ps

    @pytest.mark.smoke
    def test_unpack_returns_values_in_order(self, params):
        """T003: unpack() returns values in the same order as requested names."""
    ...
    @pytest.mark.smoke
    def test_unpack_with_single_parameter(self, params):
        """T004: unpack() works with a single parameter name."""
        (x,) = params.unpack("x")

        assert x == 1.5

    @pytest.mark.smoke
    def test_unpack_with_empty_args_returns_empty_tuple(self, params):
        """T005: unpack() with no arguments returns empty tuple."""
    ...
    @pytest.mark.smoke
    def test_unpack_raises_keyerror_for_missing_parameter(self, params):
        """T006: unpack() raises KeyError for non-existent parameter names."""
        with pytest.raises(KeyError) as exc_info:
            params.unpack("x", "nonexistent", "tau0")

        assert "nonexistent" in str(exc_info.value)

    @pytest.mark.smoke
    def test_unpack_error_message_includes_available_names(self, params):
        """T007: KeyError message lists available parameter names."""
        with pytest.raises(KeyError) as exc_info:
            params.unpack("missing_param")

        error_msg = str(exc_info.value)
        # Error message should mention available parameters for debugging
        assert "x" in error_msg or "Available" in error_msg or "available" in error_msg

    def test_unpack_with_duplicate_names_returns_duplicates(self, params):
        """T007a: unpack() with duplicate names returns the same value multiple times."""
        x1, x2, G0 = params.unpack("x", "x", "G0")
    ...
    def test_unpack_with_none_value_returns_none(self):
        """T007b: unpack() returns None as-is for parameters with None values."""
        ps = ParameterSet()
        ps.add("nullable", value=None)
        ps.add("normal", value=5.0)

        nullable, normal = ps.unpack("nullable", "normal")

        assert nullable is None
        assert normal == 5.0

    def test_unpack_with_all_param_types(self):
        """T007c: unpack() works with all parameter types (float, int, bounded)."""
        ps = ParameterSet()
        ps.add("float_param", value=3.14)
        ps.add("int_param", value=42)
        ps.add("bounded_param", value=0.5, bounds=(0.0, 1.0))

        float_val, int_val, bounded_val = ps.unpack(
            "float_param", "int_param", "bounded_param"
        )

        assert float_val == 3.14
        assert int_val == 42
        assert bounded_val == 0.5
