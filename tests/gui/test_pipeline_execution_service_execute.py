...
def test_execute_runs_nlsq_then_nuts_fit_step(qtbot):
    ...
    assert fit_result.nuts.status in (
        "completed",
        "failed",
    )  # real subprocess MCMC -- allow
...
def test_execute_runs_transform_step(qtbot):
    ...
    data = RheoData(
        x=[0.1, 1.0, 10.0], y=[1.0, 2.0, 3.0], initial_test_mode="oscillation"
    )
    lib.store_payload("d1", data)
    ...
    steps = [
        PipelineStepConfig(
            id="s1", step_type="transform", config={"name": "smooth_derivative"}
        )
    ]
    result = svc.execute(
        steps=steps,
...
def test_execute_runs_export_step(qtbot, tmp_path):
    ...
    steps = [
        PipelineStepConfig(
            id="s1", step_type="export", config={"path": out_path, "format": "csv"}
        )
    ]
    result = svc.execute(
        steps=steps,
...
def test_execute_returns_cancelled_when_stop_requested_set(qtbot):
    ...
    stop.set()
    steps = [
        PipelineStepConfig(id="s1", step_type="export", config={"path": "unused.csv"})
    ]
    result = svc.execute(
        steps=steps,
        initial_context={"data": data, "dataset_id": "d1"},
...
def test_transform_output_persisted_when_export_step_follows(qtbot, tmp_path):
    ...
    steps = [
        ...
        PipelineStepConfig(
            id="s2",
            step_type="export",
            config={"path": str(tmp_path / "out.csv"), "format": "csv"},
        ),
    ]
    result = svc.execute(
        steps=steps,
...
def test_terminal_transform_after_unrelated_fit_step_is_not_persisted(qtbot):
    ...
    steps = [
        ...
        PipelineStepConfig(
            id="s2", step_type="transform", config={"name": "smooth_derivative"}
        ),
    ]
    result = svc.execute(
        steps=steps,
...
def test_execute_reports_failed_when_nlsq_phase_fails(qtbot, monkeypatch):
    ...
    monkeypatch.setattr(
        process_adapter, "make_fit_worker", lambda **kwargs: _FakeFailingWorker()
    )
...
def test_duplicate_step_ids_rejected(qtbot):
    ...
    with pytest.raises(ValueError, match="duplicate"):
        svc.execute(
            steps=steps,
            initial_context={"data": object(), "dataset_id": "d1"},
            library=lib,
            stop_requested=threading.Event(),
        )
