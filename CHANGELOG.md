...
- **Fixed** `rheojax batch --parallel` was a documented no-op. It now loads files concurrently (`--workers` controls the pool size) while keeping `model.fit()` calls sequential, since JAX JIT compilation is not thread-safe.
- **Fixed** `r2_complex_components()` (`rheojax.utils.metrics`) returned `-1.0` for a non-perfect fit over constant (zero-variance) data, diverging from `compute_fit_quality()`'s `0.0` convention for the same condition. Both now return `0.0`.
- **Fixed** `FractionalBurgersModel._predict_relaxation`, `FractionalZenerLiquidLiquid._predict_creep`, `FractionalJeffreysModel._predict_creep`, and `FractionalZenerSolidLiquid._predict_creep` previously substituted hand-tuned heuristic blends (tanh/sigmoid crossovers, an invented `avg_order=(alpha+gamma)/2`) for the side of the G(t)/J(t) pair that has no elementary closed form, violating the linear-viscoelastic convolution identity `int_0^t G(tau)J(t-tau)dtau = t` by 17%-1574% depending on model and parameters. Replaced with a JIT-safe, differentiable fixed-grid Prony/retardation-series fit against the model's own exact complex modulus `G*(w)` (via `rheojax.utils.prony.fit_creep_retardation_series`/`fit_relaxation_prony_series`, Park & Schapery 1999), which now holds the convolution identity to within ~2% across 5 decades of time (worst case ~11% at the extreme edge for Burgers). `fit(test_mode='creep')`/`fit(test_mode='relaxation')` on these four models is now reliable.
- **Fixed** `validate_predict_input` (`rheojax/core/_validation.py`) enforced monotonic non-decreasing ordering on every `predict()` call regardless of test mode. For `ROTATION`/`FLOW_CURVE` protocols the input axis is shear rate, not time — real rheometer sweeps (up/down hysteresis loops, unsorted rate lists) are legitimately non-monotonic there, and the check rejected them (`ValueError`). `BaseModel.predict()` now skips the monotonicity check for resolved `ROTATION`/`FLOW_CURVE` test modes and for the `rheojax.models.flow.*` family (Bingham, Cross, Carreau, CarreauYasuda, HerschelBulkley, PowerLaw), whose `_predict()` never reads test mode and can be called before `fit()`.
