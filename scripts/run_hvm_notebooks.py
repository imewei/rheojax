#!/usr/bin/env python
"""Run HVM notebook suite with long timeouts and detailed logging.

Usage:
    python scripts/run_hvm_notebooks.py                     # Run all HVM notebooks
    python scripts/run_hvm_notebooks.py 01 04               # Run specific notebooks (prefix match)
    python scripts/run_hvm_notebooks.py --timeout 3600      # Custom timeout per notebook (seconds)
    python scripts/run_hvm_notebooks.py --single 06         # Run single notebook with verbose output
"""
import argparse
import datetime
import io
...
HVM_DIR = Path(__file__).resolve().parent.parent / "examples" / "hvm"
LOG_DIR = HVM_DIR / "_run_logs"
...
def discover_notebooks(prefixes: list[str] | None = None) -> list[Path]:
    """Return sorted list of HVM notebooks, optionally filtered by prefix."""
    nbs = sorted(HVM_DIR.glob("*.ipynb"))
...
class NotebookResult:
    ...
    def as_dict(self) -> dict:
        return {
            "notebook": str(self.path.relative_to(HVM_DIR.parent.parent)),
            "status": self.status,
...
def run_notebook(nb_path: Path, timeout: int = 172800) -> NotebookResult:
    ...
    try:
        ...
        # Scan cell outputs for warnings/errors from the kernel
...
def write_master_log(results: list[NotebookResult], log_dir: Path) -> None:
    ...
    with open(master_path, "w") as f:
        f.write(f"HVM Notebook Suite — {_ts()}\n")
        f.write("=" * 70 + "\n")
        f.write(f"Total: {len(results)} | PASS: {passed} | FAIL: {failed} | TIMEOUT: {timeout_count}\n")
        f.write(f"Total runtime: {total_time:.1f}s ({total_time / 3600:.2f}h)\n\n")

        for r in results:
            status_icon = {"PASS": "✓", "FAIL": "✗", "TIMEOUT": "⏱"}.get(r.status, "?")
            f.write(f"  {status_icon} {r.path.name:<50s} {r.status:<8s} {_elapsed(r.runtime):<10s} warnings={len(r.warnings)}\n")
            if r.error_summary:
...
def write_issue_inventory(results: list[NotebookResult], log_dir: Path) -> None:
    ...
    with open(inv_path, "w") as f:
        f.write("# HVM Notebook Issue Inventory\n\n")
        ...
        for r in results:
            ...
            f.write(f"- **Runtime**: {_elapsed(r.runtime)}\n")
            ...
            f.write("  ```bash\n")
            f.write(f"  python scripts/run_hvm_notebooks.py --single {r.path.stem[:2]}\n")
            f.write("  ```\n\n")
...
def main():
    parser = argparse.ArgumentParser(description="Run HVM notebook suite")
    parser.add_argument("prefixes", nargs="*", help="Notebook prefixes to filter (e.g., 01 04)")
    ...
    if args.single:
        ...
        print(f"\nStatus: {result.status} | Runtime: {_elapsed(result.runtime)} | Warnings: {len(result.warnings)}")
        if result.error_tb:
    ...
    print(f"HVM Notebook Suite — {_ts()}")
    ...
    timeout_count = sum(1 for r in results if r.status == "TIMEOUT")
    print(f"\n{'=' * 70}")
    print(f"FINAL: {passed} PASS | {failed} FAIL | {timeout_count} TIMEOUT out of {len(results)}")
    print(f"{'=' * 70}")

    return 0 if failed == 0 and timeout_count == 0 else 1
